...
void ClientSession::set_taskstatus()
{
	...
	// Проверяем на кого назначена задача.
	const int user_id = request_manager_ptr->get_task_user_id_by_task_id(task_id);

	// Пользователь может изменять статус только своей задачи.
...
void ClientSession::set_taskuser()
{
	...
	// Пользователь может назначать задачу только на себя.
...
void ClientSession::set_taskdeadline()
{
	...
	// Пользователь может изменять deadline только своей задачи.
	if (logged_user_type == UserType::User && logged_user_id != user_id) {
		request_manager_ptr->free_access(); // Освобождаем доступ к базе.
